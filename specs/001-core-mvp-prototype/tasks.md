...
- [ ] T085 **[Deferred — Next Development, `specs/004-defer-lumera-atlas/spec.md`]** Make Lumera Atlas (or an equivalent peta node visual) the reachable homepage/entry point of the shipped app, replacing or absorbing into `StudentApp`'s current `HomeScreen`, so FR-001/FR-002 and US1/AC1-2 are actually satisfiable by a user of the running app per FR-001 (contradicts) — deliberately NOT required for the current release; see T086/T087 below for how the core loop ships without it.
- [X] T086 Wire `LessonShell` and the registered modules (math-slope, physics-motion, econ-supply-demand, history-causal-chain) into a reachable route in `StudentApp` so a student can actually complete the 7-step lesson flow end-to-end, instead of the static "hadir pada batch berikutnya" info drawer currently shown by `IntegerCourseScreen`/`StudentApp.onOpenModule` (and locked in by `tests/unit/student-app.test.tsx` "opens module information but never a lesson player") per FR-003 (missing) — **DONE via `specs/004-defer-lumera-atlas` US1 (T006-T012)**: wired through `Beranda`/`Belajar`/`KursusDetail` (not Atlas, not `IntegerCourseScreen` — that legacy path is explicitly untouched, see the annotation above the "opens module information..." test). Verified by `tests/unit/loop-inti-tanpa-atlas.test.tsx`.
- [X] T087 Connect the live `HomeScreen`/`ProgressScreen` to the persisted `Siswa` record in `src/progress/store.ts` (lumens, streak, mastery) instead of only the static `ARDI_DEMO_FIXTURE`, so completing a real lesson (once T086 lands) visibly updates what the student sees outside demo mode per FR-007 (missing) — **DONE via `specs/004-defer-lumera-atlas` US1**: `Beranda` (replacing `HomeScreen` for non-demo) reads `bacaSiswa()` directly; `StudentApp.tutupPelajaran()` refreshes it after every lesson. `ProgressScreen` itself is unchanged (out of spec 004's scope — it already read `demoData`/`profile`, not blocked by this).
- [ ] T088 Re-run SC-001 through SC-005 against the running app once T085-T086 land, since they measure the Atlas-to-lesson-completion flow that is currently unreachable and therefore unverifiable per SC-001 (missing) — **partially superseded**: the non-Atlas equivalent (entry point → lesson completion, spec 001 SC-002 as revised by `specs/004-defer-lumera-atlas`) is now verified (`tests/unit/loop-inti-tanpa-atlas.test.tsx`). The literal "Atlas → completion" measurement remains blocked on T085 and stays deferred alongside it — re-run this once Atlas itself ships.
- [ ] T089 Decide and record whether to integrate or remove each UI generation left orphaned with no reachable route (`src/atlas/`, `src/beranda/`, `src/courses/`), so the repository has one authoritative home/navigation implementation instead of three coexisting, unreferenced ones per plan: project structure (unrequested) — **decided by `specs/004-defer-lumera-atlas`**: `src/beranda/` and `src/courses/` are now integrated (no longer orphaned, see T086/T087 above). `src/atlas/` decision is explicit: **keep as dormant code for next development**, not deleted, not integrated yet (Deferred, same as T085). `src/shell/HeaderNav.tsx` and `src/progress/ProgressSummary.tsx` were found to be a *third* orphaned generation during this work (nav/progress-summary alternatives) — they remain undecided, tracked separately (research.md Decision 2 in spec 004), not resolved by this task.
