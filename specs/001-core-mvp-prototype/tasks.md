...
- [ ] T085 Make Lumera Atlas (or an equivalent peta node visual) the reachable homepage/entry point of the shipped app, replacing or absorbing into `StudentApp`'s current `HomeScreen`, so FR-001/FR-002 and US1/AC1-2 are actually satisfiable by a user of the running app per FR-001 (contradicts)
  **DEFERRED 2026-08-09**: explicitly postponed by product decision — replacing/absorbing the Batch 1 `HomeScreen` (built across many recent commits) is a significant, hard-to-reverse UX call that needs its own product-direction decision, not something to default into while unblocking T086. T086 was de-risked and shipped without it (Atlas is reachable via a new `Peta Ilmu` route, not literally the homepage). FR-001 remains unmet until this is picked back up.
- [X] T086 Wire `LessonShell` and the registered modules (math-slope, physics-motion, econ-supply-demand, history-causal-chain) into a reachable route in `StudentApp` so a student can actually complete the 7-step lesson flow end-to-end, instead of the static "hadir pada batch berikutnya" info drawer currently shown by `IntegerCourseScreen`/`StudentApp.onOpenModule` (and locked in by `tests/unit/student-app.test.tsx` "opens module information but never a lesson player") per FR-003 (missing)
  **DONE 2026-08-09**: added a new `peta-ilmu` route (`src/student/routes.ts`) rendering `Atlas` (previously orphaned), unlocked the existing but disabled "Peta Ilmu" nav entry in `StudentShell.tsx`, and wired `Atlas.onPilihModul` to a new `lesson` route that mounts the real `LessonShell` with the module resolved from `src/shell/registry.ts` (`src/student/StudentApp.tsx`). Completing a lesson refreshes `Siswa` from `src/progress/store.ts` and returns to Peta Ilmu. **Scope note**: the Batch 1 integer catalog (`IntegerCourseScreen`, "Bilangan di Bawah Nol"/"Operasi Bilangan Bulat") still shows the placeholder drawer — it was left untouched on purpose, since none of the four registered spec-001 modules are about integers; wiring that specific drawer would require fabricating a false content match. The four real modules are reachable via Peta Ilmu instead. Regression tests: `tests/unit/student-app.test.tsx` ("opens a REAL LessonShell from Peta Ilmu", "falls back gracefully when a lesson link points at an unregistered module"), `tests/unit/student-routes.test.ts` unaffected (new route fields are additive).
